    ...
    >
       <v-card class="cardExtend">
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text :disabled="!showFinalPrice" @click="extendReservation">Confirm</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
</template>
...
export default {
    ...
    methods:{
        showPrice(){
            axios.post(`http://localhost:8080/reservation/extend?numberOfDays=${this.days}&idRes=${this.$props.idRes}&fromDate=${moment(this.$props.untilDate).format('DD/MM/yyyy')}`)
            .then((response) => {
